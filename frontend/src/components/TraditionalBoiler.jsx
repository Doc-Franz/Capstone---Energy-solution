...
const TraditionalBoiler = () => {
  ...
  useEffect(() => {
    dispatch(buildProductsPage("traditionalBoiler"));
  }, []);

  const traditionalBoiler = useSelector((state) => state.allProducts.content);

  return (
    <>
      <MyNavbar />
      <Container fluid className="hero" style={{ marginTop: "100px", paddingTop: "100px" }}>
        {traditionalBoiler.length > 0 ? (
          <>
            <Row className="fs-1 mb-4 fw-bold text-center mt-3">
              <Col>Sistemi con caldaia tradizionale</Col>
            </Row>
            <Container>
              <Row>
                {traditionalBoiler.map((product) => (
...
export default TraditionalBoiler;
